...
function createLeaf() {
  ...
  leaf.style.left = Math.random() * 100 + "vw"; // random start
...
window.addEventListener("scroll", function () {
  ...
  if (!ticking) {
    window.requestAnimationFrame(() => {
      ...
      // Background parallax (clamped para hindi tumatalbog)
      hero.style.backgroundPosition = `center ${lastScrollY * 0.4}px`;

      // Content parallax (gumagalaw pababa habang scroll)
      content.style.transform = `translate(-50%, calc(-50% + ${lastScrollY * 0.3}px))`;
